...
## 2026-07-02 (6) — Puan ekonomisi orantılı + merkezîleştirme

Deflasyon sonrası dağınık/sabit-kodlanmış ödüller merkezî tablolara bağlandı ve
tüm değerler orantılı (1–9, "en küçük eylem=1") hale getirildi:
- **Yeni merkezî anahtarlar** (`VARSAYILAN_PUANLAR`): `neden_bonus`(1),
  `test_soru_basi`(1)+`test_soru_max`(3), `icerik_tamamla_max`(5).
- **Şişme düzeltmeleri:** `gelisim.py` "her soru +2 max 20"→`+1/soru max 3`;
  içerik tamamlama `×10`→`×5`; "neden" bonusu sabit 3→tablo(1).
- **Sabit→tablo:** `ai_bilgi_tabani.py` kelime/soru-zengin +5 (4 yer) →
  `AI_EGITIM_PUANLARI`; sabit `puan=20` → `pdf_yukle`(5).
- **Öğrenci XP orantısı:** `ai_uretim.py` mini-oyun ödülü `3/5/7/10`→`1/2/3/4`;
  oyun display xp 5/7→4; materyal 5→3. `egzersiz.py` fallback 10→2.
